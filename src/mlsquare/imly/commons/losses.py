...
def tf_lda_loss(n_components, margin):
    """
    The main loss function (inner_lda_objective) is wrapped in this function due to
    the constraints imposed by Keras on objective functions
    """
    def tf_inner_lda_objective(y_true, y_pred):
        """
        It is the loss function of LDA as introduced in the original paper.
        It is adopted from the the original implementation in the following link:
        https://github.com/CPJKU/deep_lda
        ...
        # groups = numpy_unique(yt) # ?
        groups, _idx = tf.unique(yt)

        def compute_cov(group, Xt, yt):
            # Xgt = Xt[T.eq(yt, group).nonzero()[0], :] # What does `nonzero` do here?
            Xgt = Xt[tf.equal(yt, group).count_nonzero()[0], :] 
            ### Chances are high that this is precisely the wrong way to use nonzero
            # Xgt_bar = Xgt - T.mean(Xgt, axis=0) # ?
            Xgt_bar = Xgt - tf.reduce_mean(Xgt, axis=0)
            # m = T.cast(Xgt_bar.shape[0], 'float32') # ?
            m = tf.cast(Xgt_bar.shape[0], tf.float32)
            # return (1.0 / (m - 1)) * T.dot(Xgt_bar.T, Xgt_bar) # ? Verify if `T` stands for transpose
            return (1.0 / (m - 1)) * tf.tensordot(tf.transpose(Xgt_bar), Xgt_bar, axes=2) # Why is axes=2?
        ...
        covs_t = tf.scan(fn=compute_cov, elems=[groups]) 
        # How to communicate `non_sequence` elements?
        ...
        m = tf.cast(Xt_bar.shape[0], tf.float32)
        St_t = (1.0 / (m - 1)) * T.dot(Xt_bar.T, Xt_bar)
        St_t = (1.0 / (m - 1)) * tf.tensordot(tf.transpose(Xt_bar), Xt_bar, axes=2)
        ...
        # compute eigenvalues
        # evals_t = T.slinalg.eigvalsh(Sb_t, Sw_t) #? 
        evals_t = tf.linalg.eigvalsh(Sb_t, Sw_t)
        # Feasibility of 2 args?
        ...
        # (k smallest eigenvalues below threshold)
        # thresh = T.min(top_k_evals) + margin #? 
        thresh = tf.reduce_min(top_k_evals) + margin

        top_k_evals = top_k_evals[(top_k_evals <= thresh).nonzero()] # ?
        costs = T.mean(top_k_evals) #?
        costs = tf.reduce_mean(top_k_evals, axis=0)

        return -costs

    return tf_inner_lda_objective
